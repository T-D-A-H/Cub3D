...
/*   Updated: 2025/04/01 13:20:49 by ctommasi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_ceiling(t_cub *cub, t_loop *loop, int x)
{
	...
	while (y < loop->drawstart)
	{
		...
		tex_x = (int)(floor_x * cub->textures[2]->width) % cub->textures[2]->width;
		tex_y = (int)(floor_y * cub->textures[2]->height) % cub->textures[2]->height;
		put_pixel(x, y, cub->textures[2]->data[tex_y * cub->textures[2]->width + tex_x], cub);
		y++;
	}
...
void	draw_floor(t_cub *cub, t_loop *loop, int x)
{
	...
	while (y < HEIGHT)
	{
		...
		floor_x = cub->player->x / BLOCK + row_distance * loop->raydir_x;
		floor_y = cub->player->y / BLOCK + row_distance * loop->raydir_y;
		tex_x = (int)(floor_x * cub->textures[1]->width) % cub->textures[1]->width;
		tex_y = (int)(floor_y * cub->textures[1]->height) % cub->textures[1]->height;
		put_pixel(x, y, cub->textures[1]->data[tex_y * cub->textures[1]->width + tex_x], cub);
		y++;
	}
...
void	draw_walls(t_cub *cub, t_loop *loop, t_draw *draw, int x)
{
	...
	while (++y < loop->drawend)
	{
		...
		draw->color = cub->textures[draw->texi]->data[cub->textures[draw->texi]->height
			* draw->texy + loop->texx];
		if (y >= 0 && y < HEIGHT)
		{
